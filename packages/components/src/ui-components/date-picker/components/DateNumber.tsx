...
export type DateNumberVariants =
  | 'currentDate'
  | 'start'
  | 'end'
  | 'selected'
...
export interface DateNumberProps
...
const DateNumberButton = React.forwardRef<HTMLButtonElement, DateNumberProps>(
  ({ children, variant = 'default', ...rest }, ref) => {
    const dateNumberButtonStyles = useComponentStyles('dateNumber', {
      variant,
    });
    const dateNumberBoxStyles = useComponentStyles('dateNumberBox', {
      variant,
    });
